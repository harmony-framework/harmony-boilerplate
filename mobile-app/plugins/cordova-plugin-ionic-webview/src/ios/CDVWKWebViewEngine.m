...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [[self keyWindow] addSubview:self.engineWebView];
...
-(NSString *) getStartPath {
    ...
    if (persistedPath && ![persistedPath isEqualToString:@""]) {
        ...
        NSString * cordovaDataDirectoryUpdateDir = [cordovaDataDirectory stringByAppendingPathComponent:persistedPath];
        self.basePath = cordovaDataDirectoryUpdateDir;
        return cordovaDataDirectoryUpdateDir;
    } else {
...
- (void)pluginInitialize
{
    ...
    NSString *customEntryPoint = [settings cordovaSettingForKey:@"customEntryPoint"];
    if(customEntryPoint == nil){
    ...
    self.handler = [[IONAssetHandler alloc] initWithBasePath:[self getStartPath] andScheme:scheme andCustomEntryPoint:customEntryPoint];
...
- (void) webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction*)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler
{
    ...
    if (shouldAllowRequest) {
        NSDictionary* settings = self.commandDelegate.settings;
        NSString *allowedUrlsAsIntent = [settings cordovaSettingForKey:@"AllowedSchemeURLsAsIntent"];
        NSString *scheme = url.scheme;
        BOOL isURLContainedAllowedIntents = NO;
        if(allowedUrlsAsIntent) {
           isURLContainedAllowedIntents = [url.absoluteString rangeOfString:allowedUrlsAsIntent].location != NSNotFound;
        }
        if ([scheme isEqualToString:@"tel"] ||
            [scheme isEqualToString:@"mailto"] ||
            [scheme isEqualToString:@"facetime"] ||
            [scheme isEqualToString:@"sms"] ||
            [scheme isEqualToString:@"maps"] || isURLContainedAllowedIntents) {
...
-(void)setServerBasePath:(CDVInvokedUrlCommand*)command
{
    ...
    NSString *libPath = [NSSearchPathForDirectoriesInDomains(NSLibraryDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSString * cordovaDataDirectory = [libPath stringByAppendingPathComponent:@"NoCloud"];
    NSString * cordovaDataDirectoryUpdateDir = [cordovaDataDirectory stringByAppendingPathComponent:path];
    [self.handler updateBasePath:cordovaDataDirectoryUpdateDir];
    self.basePath = cordovaDataDirectoryUpdateDir;
    [self.handler setAssetPath:cordovaDataDirectoryUpdateDir];
...
-(void)getUserConfiguration:(CDVInvokedUrlCommand*)command
{
    NSDictionary* settings = self.commandDelegate.settings;
    NSString *contentUpdateURL = [settings cordovaSettingForKey:@"contentUpdateURL"];
    NSString *contentUpdateEnabled = [settings cordovaSettingForKey:@"contentUpdateEnabled"];
    NSString *applicationID = [settings cordovaSettingForKey:@"ApplicationID"];

    NSDictionary *userConfiguration = @{ @"contentUpdateURL" : contentUpdateURL, @"contentUpdateEnabled" : contentUpdateEnabled, @"appId" :applicationID };

    [self.commandDelegate sendPluginResult:[CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsDictionary:userConfiguration]  callbackId:command.callbackId];
