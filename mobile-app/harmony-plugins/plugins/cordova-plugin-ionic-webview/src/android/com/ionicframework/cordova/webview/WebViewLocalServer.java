...
public class WebViewLocalServer {
  ...
  WebViewLocalServer(Context context, String authority, boolean html5mode, ConfigXmlParser parser, String customScheme, String customEntryPoint) {
  ...
  private WebResourceResponse handleLocalRequest(Uri uri, PathHandler handler, WebResourceRequest request) {
    ...
    if (path.equals("") || path.equals("/") || (!uri.getLastPathSegment().contains(".") && html5mode)) {
      ...
      try {
        ...
        if(launchURL.contains(this.customEntryPoint)) {
            startPath = this.basePath + "/" + this.customEntryPoint;
        }
        if (isAsset) {
