...
public class IonicWebViewEngine extends SystemWebViewEngine {
  ...
  private String customEntryPoint = "index.html";
  ...
  public void init(CordovaWebView parentWebView, CordovaInterface cordova, final CordovaWebViewEngine.Client client,
    ...
    Common.assetManager = cordova.getActivity().getApplicationContext().getAssets();

    String hostname = preferences.getString("Hostname", "localhost");
    scheme = preferences.getString("Scheme", "http");
    customEntryPoint = preferences.getString("CustomEntryPoint", "index.html");
    CDV_LOCAL_SERVER = scheme + "://" + hostname;

    localServer = new WebViewLocalServer(cordova.getActivity(), hostname, true, parser, scheme, customEntryPoint);
  ...
  private class ServerClient extends SystemWebViewClient {
    ...
    public boolean shouldOverrideUrlLoading(WebView view, String url) {
      String allowedUrlsAsIntents = preferences.getString("AllowedSchemeURLsAsIntent", "");
      if(!allowedUrlsAsIntents.equals("") && url.contains(allowedUrlsAsIntents)) {
