...
- (void)copyConentSync:(NSString *) sourceDir andDest: (NSString *) destDir
{
    ...
    for (NSString *currentFile in sourceFiles)
    {
        ...
        NSLog(@"Iterating files: %@", fullFilePath);
        if ([fileManager fileExistsAtPath:fullFilePath isDirectory:&isDirectory] && !isDirectory)
        {
            if ([fileManager fileExistsAtPath:[destDir stringByAppendingPathComponent:currentFile]] == YES) {
                [fileManager removeItemAtPath:[destDir stringByAppendingPathComponent:currentFile] error:&removeError];
            }
            ...
            [fileManager copyItemAtPath:sourceDir toPath:destDir error:&copyError];
        }
    }
    [fileManager removeItemAtPath:sourceDir error:&deleteError];
}
    ...
    [self.commandDelegate runInBackground:^{
        ...
        @try {
            ...
            if(![SSZipArchive unzipFileAtPath:[sourceURL path] toDestination:[destinationURL path] overwrite:YES password:nil error:&error delegate:weakSelf]) {
                ...
                NSFileManager *fileManager = [NSFileManager defaultManager];
                [fileManager removeItemAtURL:sourceURL error:NULL];
                //NSString *theFileName = [[sourceURL lastPathComponent] stringByDeletingPathExtension];

                //NSString * sourceURLContent = [[destinationURL path] stringByAppendingPathComponent:theFileName];

                //[self copyConentSync:sourceURLContent andDest:[destinationURL path]];
...
- (void)zipArchiveProgressEvent:(long)loaded total:(long)total {
